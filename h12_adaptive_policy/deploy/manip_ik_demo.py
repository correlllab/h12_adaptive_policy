...
def main():
    ...
    if args.sweep:
        payloads = list(range(1, 4))   # trained range only: 30 N ~= 3 kg
        results = {"FAME": [], "no-FAME": []}; trajs = {}
        print(f"\n[sweep] {args.task}: payloads {payloads} kg")
        for kg in payloads:
            for lab, no_enc in [("FAME", False), ("no-FAME", True)]:
                mt, _ = run_manip(config, m, ids, policy, encoder, bounds, side=side, rm=rm, xyz_at=xyz_at,
                                  total_s=total, payload_kg=float(kg), settle_s=settle_s,
                                  load_ramp_s=ramp_s, torso=torso, no_encode=no_enc)
                results[lab].append(mt)
                if kg == args.plot_payload:
                    trajs[lab] = mt["traj"]
                print(f"  {kg}kg {lab:8s} e^W_ee={mt['ee_rmse']*100:5.1f}cm  "
                      f"e^W_base={mt['ee_b_rmse']*100:4.1f}cm  base={mt['base_rmse']*100:4.1f}cm  fell={mt['fell']}")
    ...
    if args.sweep_video:
        payloads = list(range(1, 4))   # trained range only
